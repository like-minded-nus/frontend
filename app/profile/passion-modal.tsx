...
const PassionModal: React.FC<ModalProps> = ({ isOpen, onClose, selectedPassion, togglePassion, passionNames }) => {
  if (!isOpen) return <></>;
    ...
    <div className='fixed left-0 top-0 z-10 flex h-full w-full items-center justify-center bg-gray-500 bg-opacity-50'>
      <div className='w-[30%] rounded-lg bg-white p-6 text-center'>
        <div className='flex flex-col items-center justify-between '>
          ...
          <div className="w-[100%]">
        {isLoading && 
        <Skeleton count={8} borderRadius={50} height={30}  style={{ marginBottom: ".6rem" }} />}
        {!isLoading &&
          passionOptions.map((passion: Passion) => (
            ...
            key={passion.passionId}>{passion.passionName}</button>
          ))}

          </div>
      <button className='btn btn-primary btn-solid self-end' onClick={() => onClose(passionsNameList)}>Done</button>
        </div>
      </div>
    </div>
  );
